            ...
            <div class="container-fluid d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
                ...
                <div class="d-flex align-items-center flex-wrap mr-1">
                    ...
                    <div class="d-flex align-items-baseline flex-wrap mr-5">
                        ...
                        <h5 class="text-dark font-weight-bold my-1 mr-5">Sửa thông tin tin tức</h5>
                        <!--end::Page Title-->
                        <!--begin::Breadcrumb-->
                        ...
                        <div class="wizard wizard-1" id="kt_wizard" data-wizard-state="first" data-wizard-clickable="false">
                            ...
                            <div class="row justify-content-center my-10 px-8 my-lg-15 px-lg-10">
                                <div class="col-xl-12 col-xxl-7">
                                    <!--begin::Wizard Form-->
                                    <form action="{{ route('admin.news.edit', ['id'=> $news->id]) }}" class="form fv-plugins-bootstrap fv-plugins-framework" method="POST"  id="kt_form">
                                    @csrf
                                    <!--begin::Wizard Step 1-->
                                        <div class="pb-0" data-wizard-type="step-content" data-wizard-state="current">
                                            <h3 class="mb-10 font-weight-bold text-dark">Nhập thông tin tin tức</h3>

                                            <!--begin::Input-->
                                            <div class="form-group fv-plugins-icon-container">
                                                <label>Tiêu đề<span class="text-danger">*</span></label>
                                                <input type="text" required class="form-control form-control-solid form-control-lg" name="title" placeholder="Nhập tên thương hiệu" value="{{ old('title', $news->title) }}">
                                                @error('title')
                                            ...
                                            <div class="form-group fv-plugins-icon-container">
                                                ...
                                                <select type="text" required class="form-control form-control-solid form-control-lg" name="category_news_id">
                                                    @if(!empty($cate))
                                                        @foreach($cate as $key => $value)
                                                            <option @if(old('category_news_id', $news->category_news_id) == $value->id) selected @endif value="{{ $value->id }}">{{ $value->name }}</option>
                                                        @endforeach
                                                    @endif
                                                </select>
                                            ...
                                            <div class="form-group">
                                                ...
                                                <textarea required name="description" id="" class="form-control form-control-solid" cols="30" rows="10" placeholder="Mô tả">{{ $news->description }}</textarea>
                                                @error('description')
                                                <div class="text-danger">{{ $message }}</div>
                                                @enderror
                                            ...
                                            <div class="form-group fv-plugins-icon-container">
                                                ...
                                                <textarea name="content" id="content" class="form-control form-control-solid form-control-lg"
                                            ...
                                            <div class="form-group fv-plugins-icon-container">
                                                <label>Ảnh<span class="text-danger">*</span></label><br>
                                                <input type="file" class="upload-file"
                                                       onchange="handleImage(this.files, 2)" >
                                                <div class="text-danger"></div>
                                                <div class="fv-plugins-message-container"></div></div>
                                            <input type="hidden" name="image" value="{{ old('image', $news->image) }}" >
                                            <img id="img" src="{{ old('image', $news->image) }}" alt="" width="100px" height="100px" style="object-fit: cover">
                                            <br>
                                            <br>
                                            <div class="form-group">
                                                <select name="status" class="form-control" id="">
                                                    @if(!empty($aryStatus))
                                                        @foreach($aryStatus as $key => $value)
                                                            <option value="{{ $key }}" @if(old('status', $news->status) == $key) selected @endif>{{ $value }}</option>
                                                        @endforeach
                                                    @endif
                                                </select>
                                            </div>
